...
void generate_tensor::get_out_meta(
  ...
  const std::uint32_t num_dims = (int64_t)params.get_int<0>();
  const std::uint32_t dim0 = (int64_t)params.get_int_or_default<1>(1);
  const std::uint32_t dim1 = (int64_t)params.get_int_or_default<2>(1);
  const std::uint32_t dim2 = (int64_t)params.get_int_or_default<3>(1);

  // get the output tensor
  auto &out = _out.get<0>().as<bert_dense_meta_t>().m();
  out = {.num_dim = 1, .dim0 = dim0, .dim1 = dim1, .dim2 = dim2};
}
...
void generate_tensor::exec(const bbts::ud_impl_t::tensor_params_t &params,
  ...
  m_out = {.num_dim = 1, .dim0 = dim0, .dim1 = dim1, .dim2 = dim2};
